...
func PreLLMHook(ctx *schemas.BifrostContext, req *schemas.BifrostRequest) (*schemas.BifrostRequest, *schemas.LLMPluginShortCircuit, error) {
    if req == nil || req.ResponsesRequest == nil || !cfg.Enabled {
        return req, nil, nil
    }
    ...
    if cfg.SkipIfRawRequestBody && len(rr.RawRequestBody) > 0 {
        debug(ctx, "skipping because RawRequestBody is already set")
        return req, nil, nil
    }
    ...
    if cfg.SkipIfRequestHasCache && anthropicReq.CacheControl != nil {
        debug(ctx, "skipping because top-level cache_control already exists after conversion")
        return req, nil, nil
    }

    anthropicReq.CacheControl = &schemas.CacheControl{
    ...
    debug(ctx, fmt.Sprintf("injected top-level cache_control for provider=%s model=%s", rr.Provider, rr.Model))
    return req, nil, nil
}
...
func debug(ctx *schemas.BifrostContext, msg string) {
	...
	// Use stdout logging so this plugin remains compatible with v1.4.22.
	fmt.Printf("[%s] %s\n", pluginName, msg)
}